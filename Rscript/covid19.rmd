...
plot_country_facet_line <- function(data, x.var, y.var, title, n.col, n.row, scales){
  y.var = enquo(y.var)
  x.var = enquo(x.var)
  ...
  gg_plot = ggplot(data = data, aes(x=!! x.var, y=!! y.var)) + 
    ...
    theme(legend.position = "none") +
    scale_color_paletteer_d("ggthemes::gdoc") +
    facet_wrap(~country, scales = scales, ncol = n.col, nrow = n.row)
  return(gg_plot)
}

plot_country_facet_line_area <- function(data, x.var, y.var, y.area, title, n.col, n.row, scales){
  y.var = enquo(y.var)
  y.area = enquo(y.area)
  x.var = enquo(x.var)
  
  gg_plot = ggplot(data = data) +
    geom_area(aes(x=!! x.var, y=!! y.area), outline.type=NULL, fill='gold2') +
    geom_line(aes(x=!! x.var, y=!! y.var), size=1.25, color='magenta4') +
    labs(title = title) + theme_fivethirtyeight() +
    theme(legend.position = "none") +
    scale_color_paletteer_d("ggthemes::gdoc") +
    facet_wrap(~country, scales = scales, ncol = n.col, nrow = n.row)
...
plot_country_facet <- function(data, x.var, y.var, n.knots, title, n.col, n.row, scales){
  y.var = enquo(y.var)
  x.var = enquo(x.var)
...
plot_country_facet_loess <- function(data, x.var, y.var, span, title, n.col, n.row, scales){
  y.var = enquo(y.var)
  x.var = enquo(x.var)
  
  gg_plot = ggplot(data = data, aes(x=!! x.var, y=!! y.var)) + 
    geom_point(size=3, color='gray20', alpha=0.5) +
    stat_smooth(se=FALSE, na.rm = TRUE, span=span, size=1.25, color='red3') + 
    labs(title = title) + theme_fivethirtyeight() +
    theme(legend.position = "none") +
    scale_color_paletteer_d("ggthemes::gdoc") +
    facet_wrap(~country, scales = scales, ncol = n.col, nrow = n.row)
...
```{r}
d = fread('https://raw.githubusercontent.com/datasets/covid-19/master/data/countries-aggregated.csv')
...
d = d %>% mutate(Date = as.Date(Date) ) %>%
  as_tibble  %>% rename(time=Date, country=Country, confirm=Confirmed, heal=Recovered, dead=Deaths) %>%  
  group_by(country) %>%
  ...
  tidyr::fill(confirm, dead, heal) %>% ungroup(country)
...
```{r}
# Select the countries with population larger than 10 000, natural earth is a nice source to fetch geo data
...
pop_est = world %>% 
  filter(region_un %in% c('Africa', 'Americas', 'Asia', 'Europe', 'Oceania') & pop_est > 10000) %>% 
  select(name, pop_est) 
st_geometry(pop_est) = NULL

pop_est = as_tibble(pop_est)

# Join to covid data
d = d %>% as_tibble %>% mutate(country = as.factor(country)) %>% 
  mutate(country =  recode(country, US = "United States"))                 
d = left_join(d, pop_est, by = c("country" = "name")) 
d$country = as.factor(d$country)

d_w = d_w %>% as_tibble %>% mutate(country = as.factor(country)) %>% 
  mutate(country =  recode(country, US = "United States"))                 
d_w = left_join(d_w, pop_est, by = c("country" = "name")) 
d_w$country = as.factor(d_w$country)

# remove countries which are not in the world SF file (Andorra, San Marino, etc.)
d = d %>% filter(complete.cases(.))
d_W = d_w %>% filter(complete.cases(.))

d = d %>% group_by(country) %>% 
  mutate(confirm_pm = 1000000*confirm/pop_est, heal_pm = 1000000*heal/pop_est, dead_pm = 1000000*dead/pop_est) %>%
  ungroup(country)

d_w = d_w %>% group_by(country) %>% 
  mutate(confirm_pm = 1000000*confirm/pop_est, heal_pm = 1000000*heal/pop_est, dead_pm = 1000000*dead/pop_est) %>%
  ungroup(country)
```
...
world_df = d %>%
  ...
  tidyr::fill(confirm, dead, heal) %>% ungroup %>% arrange(country, time)
...
world_df_heal = d %>% 
  as_tibble  %>% 
  filter(heal >= 1) %>% arrange(country, time) %>%
  tidyr::complete(country, time) %>% group_by(country) %>% 
  tidyr::fill(confirm, dead, heal) %>% ungroup %>% arrange(country, time)
...
world_w_df = d_w %>%
  ...
  tidyr::fill(confirm, dead, heal) %>% ungroup %>% arrange(country, week)
...
world_w_df_heal = d_w %>%
  as_tibble  %>% 
  filter(heal >= 1) %>% arrange(country, week) %>%
  tidyr::complete(country, week) %>% group_by(country) %>% 
  tidyr::fill(confirm, dead, heal) %>% ungroup %>% arrange(country, week)
...
Obviously the answer is no, for Belgium at least. Is the forthcoming boom of the unemployment rate worth the price? What would be the benefit of the actual virus management? 
All those questions are delicate and deserve a profound reflection. I'll not dare to answer but I'll just provide the magnitude order so far of the virus compared to the overall yearly mortality for some countries.
...
raw_ranking_plot = ggplot(data = last_df, aes(x=reorder(country, death_frac), y=100*death_frac, color=death_frac)) +
  geom_point(size = 2.5) + theme_fivethirtyeight() +  
  coord_flip() +
  scale_color_gradientn(colours = pals::kovesi.rainbow(100), guide = FALSE) +
  labs(subtitle = 'Percentage of the total number of deaths in 2018', title = 'Covid-19 mortality fraction') 
raw_ranking_plot
```
...
```{r}
world_ex = fread('https://raw.githubusercontent.com/TheEconomist/covid-19-excess-deaths-tracker/master/output-data/excess-deaths/all_weekly_excess_deaths.csv')

country_pop = world_ex %>% select(country, population) %>% 
  distinct(country, population, .keep_all = FALSE) %>% 
  group_by(country) %>%
  filter(population == max(population)) %>% 
  arrange(country, population)

world_ex = world_ex %>% select(country, year, week, total_deaths, expected_deaths, covid_deaths, excess_deaths, end_date, excess_deaths_per_100k)

world_eu_monthly = world_ex %>% mutate(date = as.Date(end_date)) %>% mutate(month = month(date)) %>%
  group_by(country, year, month) %>% 
  summarise_if(is.numeric, sum, na.rm = TRUE) %>% 
  ungroup() %>% mutate(excess_expected = excess_deaths/expected_deaths)

world_eu_monthly = left_join(world_eu_monthly, country_pop, by = c("country" = "country") ) %>% mutate(excess_pm = 1000000*excess_deaths/population)

world_eu_weekly = world_ex %>% 
  group_by(country, year, week) %>% 
  summarise_if(is.numeric, sum, na.rm = TRUE) %>% 
  ungroup() %>% mutate(excess_expected = excess_deaths/expected_deaths)

world_eu_weekly = left_join(world_eu_weekly, country_pop, by = c("country" = "country") ) %>% mutate(excess_pm = 1000000*excess_deaths/population)
```

### Absolute numbers per million inhabitants

```{r, fig.height=5, fig.width=5.25}
plot_country_facet_line(data=world_eu_monthly %>% filter(!country %in% c('Mexico', 'Chile', 'South Africa')), 
                        x.var=month, 
                        y.var=excess_pm, 
                        title='Excess deaths per million inhabitants', 
                        n.col=4, 
                        n.row=5,
                        scales='fixed')
```
...
```{r, fig.height=5, fig.width=5.25}
excess_exp = plot_country_facet_line(data=world_eu_monthly %>% 
                                       filter(!country %in% c('Mexico', 'Chile', 'South Africa')), 
                                     x.var=month, 
                                     y.var=100*excess_expected, 
                                     title='Excess deaths divided by expected (%)', 
                                     n.col=4, 
                                     n.row=5,
                                     scales='fixed')
excess_exp
...
plot_country_facet_loess(data=world_eu_weekly %>% filter(!country %in% c('Mexico', 'Chile', 'South Africa')), 
                   x.var=week, 
                   y.var=100*excess_expected, 
                   span=.5, 
                   title='Weakly excess divided by expected deaths (%)', 
                   n.col=4, 
                   n.row=5,
                   scales='fixed')
```
...
```{r, fig.height=6, fig.width=6}
ex_cov = plot_country_facet_line_area(data=world_eu_monthly %>% 
                                        filter(!country %in% c('Mexico', 'Chile', 'South Africa')), 
                                      x.var=month, 
                                      y.var=excess_deaths, 
                                      y.area=covid_deaths,
                                      title='Excess (line) and covid (area) deaths', 
                                      n.col=4, 
                                      n.row=5,
                                      scales='free')
ex_cov
...
```{r, fig.height=6, fig.width=6}
ex_cov_W = plot_country_facet_line_area(data=world_eu_weekly %>% 
                                        filter(!country %in% c('Mexico', 'Chile', 'South Africa')), 
                                      x.var=week, 
                                      y.var=excess_deaths, 
                                      y.area=covid_deaths,
...
healed_pm = plot_top_states(data = d, 
                            x.var=days_since_100,
                            y.var = heal_pm, 
                            color.var=country,
                            n.state = 5L,
